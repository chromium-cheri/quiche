...
TEST(HttpEncoderTest, SerializePriorityUpdateFrame) {
  ...
  quiche::test::CompareCharArraysWithHexError(
      "PRIORITY_UPDATE", buffer.get(), length,
      reinterpret_cast<const char*>(output1), ABSL_ARRAYSIZE(output1));
}
...
TEST(HttpEncoderTest, SerializeAcceptChFrame) {
  ...
  EXPECT_EQ(ABSL_ARRAYSIZE(output1), length);
  quiche::test::CompareCharArraysWithHexError(
      "ACCEPT_CH", buffer.get(), length, reinterpret_cast<const char*>(output1),
  ...
  quiche::test::CompareCharArraysWithHexError(
      "ACCEPT_CH", buffer.get(), length, reinterpret_cast<const char*>(output2),
