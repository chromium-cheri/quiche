...
TEST_P(QuicDispatcherTestAllVersions,
  ...
  QuicReceivedPacket packet(reinterpret_cast<const char*>(short_packet), 21,
                            QuicTime::Zero());
  uint8_t valid_size_packet[23] = {0x70, 0xa7, 0x02, 0x6c};
  QuicReceivedPacket packet2(reinterpret_cast<const char*>(valid_size_packet),
                             23, QuicTime::Zero());
  EXPECT_CALL(*dispatcher_, CreateQuicSession(_, _, _, _, _, _)).Times(0);
...
TEST_P(QuicDispatcherTestOneVersion,
  ...
  QuicReceivedPacket received_packet(reinterpret_cast<const char*>(packet),
...
TEST_P(QuicDispatcherTestOneVersion,
  ...
  QuicReceivedPacket received_packet44(reinterpret_cast<const char*>(packet44),
...
TEST_P(QuicDispatcherTestAllVersions, ProcessSmallCoalescedPacket) {
  ...
  // clang-format on
  QuicReceivedPacket packet(reinterpret_cast<const char*>(coalesced_packet),
                            1200, QuicTime::Zero());
  dispatcher_->ProcessPacket(server_address_, client_address, packet);
}
